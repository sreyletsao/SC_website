<template>
  <div class="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 p-6">
    <div class="max-w-7xl mx-auto space-y-8">
        ...
        <div>
          <div class="flex items-center gap-3 mb-2">
            ...
            <div>
              ...
              <p class="text-gray-600">Manage your organization's leadership team</p>
            </div>
          </div>
        </div>
      ...
      <div class="bg-white/80 backdrop-blur-sm rounded-xl shadow-lg border-0 p-6">
        <div class="flex flex-col lg:flex-row gap-4 items-center justify-between">
          <div class="flex flex-col sm:flex-row gap-4 flex-1">
            ...
            <button class="px-4 py-3 bg-white border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors flex items-center gap-2">
      ...
      <!-- Enhanced Table -->
      <div class="bg-white/90 backdrop-blur-sm rounded-xl shadow-xl border-0 overflow-hidden">
        <div class="overflow-x-auto">
          <table class="w-full">
            <thead>
              <tr class="bg-gradient-to-r from-gray-50 to-gray-100 border-b-2 border-gray-200">
                <th class="p-4 font-semibold text-gray-700 text-center w-16">ID</th>
                <th class="p-4 font-semibold text-gray-700 text-center w-20">Photo</th>
                <th class="text-left p-4 font-semibold text-gray-700">Name</th>
                <th class="text-left p-4 font-semibold text-gray-700">Position</th>
                <th class="text-left p-4 font-semibold text-gray-700">Description</th>
                <th class="p-4 font-semibold text-gray-700 text-center w-20">Actions</th>
              </tr>
            </thead>
            <tbody>
              <tr 
                ...
                :key="member.id" 
                :class="[
                  'hover:bg-blue-50/50 transition-colors duration-200 border-b border-gray-100',
                  index % 2 === 0 ? 'bg-white' : 'bg-gray-50/30'
                ]"
              >
                <td class="p-4 text-center font-medium text-gray-600">#{{ member.id }}</td>
                ...
                <td class="p-4 text-center relative">
                  ...
                  >
                    <button
                      @click="() => { openForm(member); closeDropdown(); }"
                    ...
                    <button
                      @click="() => { alert('Viewing ' + member.name); closeDropdown(); }"
                    ...
                    <button
                      @click="() => { deleteMember(member.id); closeDropdown(); }"
        ...
        <div v-if="members.length === 0" class="text-center py-16">
          ...
          <h3 class="text-xl font-medium text-gray-900 mb-2">No board members found</h3>
          <p class="text-gray-600 mb-6">Get started by adding your first board member to the system.</p>
          <button
            @click="openForm()"
          ...
          <form @submit.prevent="saveMember" class="space-y-6">
            ...
            <div>
              ...
              <div class="flex items-center space-x-4">
                ...
                />
                <div
                  v-if="editingMember.image && typeof editingMember.image === 'string'"
                  class="flex-shrink-0"
                >
                  <img
...
const fetchMembers = async () => {
  try {
    ...
    members.value = res.Team; // adjust if your backend returns differently
    // Log all members and their image paths for debugging
    members.value.forEach((member) => {
      //   console.log('Member:', member.name, 'Image path:', member.image)
      //   console.log('Resolved URL:', getImageUrl(member.image))
    });
  } catch (err) {
...
async function saveMember() {
  try {
    ...
    if (editingMember.value.image instanceof File) {
      ...
      //   console.log('Appending image file to FormData:', editingMember.value.image.name)
    } else {
      console.log("No new image file selected or image is a URL string");
    }
